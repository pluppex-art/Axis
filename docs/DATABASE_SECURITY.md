...
- **Permissão por módulo em modo log:** os triggers `trg_permission_log_{crm,financeiro,rh,clinica,educacao}` chamam `log_module_permission_check`, que grava em `permission_check_log` (`would_have_blocked`) **sem bloquear nada**. O enforcement real hoje é só a leitura de clínica/educação via `user_has_module_access` (CR3, aplicado em 2026-09-24); CRM, financeiro e RH continuam só em modo log.
