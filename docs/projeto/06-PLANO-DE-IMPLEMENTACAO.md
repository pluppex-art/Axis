...
| 1.6 | **Reconciliar migrations não aplicadas** (`a1`, `cr1`, `cr2`, `cr3`) | 1,5 | Cada migration revisada, testada em **branch Supabase**, aplicada ou removida do repo; `guard_finance_period_lock` ativo no banco; advisors sem alerta novo |
...
| 4.9 | 🚨 **`resolveTenantId` do Google Calendar** (`server/googleCalendar.ts` ~165–171) devolve o tenant pedido em `x-active-tenant-id` **mesmo quando `has_tenant_access` nega** → responder 403 | P0 | 0,5 | Header com tenant alheio recebe 403; teste automatizado |
| 4.10 | 🚨 **SSRF autenticado** em `/api/integrations/webhook-test` e `/smtp-test`: bloquear loopback/IPs privados/metadata (`169.254.169.254`), validar DNS resolvido, limitar redirects | P0 | 1,5 | Destinos internos rejeitados; teste com IPs privados |
| 4.11 | `POST /api/public-proposal/:token/accept`: conferir status atual (só `Enviada`/`Aberta`), `validade`, `view_count` atômico, rate limit, gravar quem aceitou | P1 | 1,5 | Proposta recusada/expirada não pode ser aceita; limite por IP/token |
| 4.12 | Cache Redis por tenant **e permissão**: incluir usuário/cargo na chave (ou não cachear) nas rotas de módulos com controle por cargo (`/api/clinica/*`, `/api/education/*`, `table-preview`) | P1 | 1 | Usuário sem permissão não recebe resposta cacheada de outro |
| 4.13 | `/api/auth/tenant-theme`: não revelar se um e-mail existe nem permitir enumerar tenants por nome; rate limit | P2 | 1 | Resposta uniforme; limitador ativo |
| 4.14 | `GET /api/admin/permission-check-log` exigir `requireMaster` (hoje só `requireUser`) e conferir que `POST /api/admin/tenant` define `is_tenant_admin=true` no admin inicial | P2 | 0,75 | 403 para não-master; admin do tenant novo enxerga telas de admin |
