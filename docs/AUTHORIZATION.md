...
Os triggers `trg_permission_log_{crm,financeiro,rh,clinica,educacao}` chamam `log_module_permission_check`, que só grava em `permission_check_log` (`would_have_blocked`) — **nenhuma escrita/leitura é negada por cargo/módulo ainda** (verificado ao vivo em 2026-09-24). A migration `20260921_cr1_*` (escrita de `cargos`/`squads` só para admin/master, via `is_tenant_admin_or_master`) e a `20260921_cr2_*` (módulos/plano do tenant só pelo master) foram **aplicadas em 2026-09-24**; a `20260921_cr3_*` (leitura de clínica/educação via `user_has_module_access`) também foi **aplicada** em 2026-09-24.
