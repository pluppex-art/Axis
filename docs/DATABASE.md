...
Supabase/Postgres 17, projeto `snwkzvgompfgqoqbpihe`. Schema `public`, **129 tabelas, todas com RLS** (verificado ao vivo em 2026-09-24). Migrações versionadas em [`supabase/migrations/`](../supabase/migrations/) — atenção: todas as `20260921_*` do repo **estão aplicadas** no banco vivo (2026-09-24). O dicionário de dados completo (tabelas, colunas, FKs, funções, triggers) está em [`projeto/05-ESQUEMA-BACKEND.md`](projeto/05-ESQUEMA-BACKEND.md), que é a fonte de verdade; este documento só resume convenções. Extensões: `plpgsql`, `uuid-ossp`, `pgcrypto`, `pg_stat_statements`, `supabase_vault`, `pg_net`, `pg_graphql`, `pg_cron`, `http`, `vector`.
